...
def _should_read_dag_from_db(subdir: str | None) -> bool:
    """Detect Airflow serialized DAG references used by Kubernetes task pods."""
    return bool(subdir and str(subdir).startswith("DAGS_FOLDER/db-json:"))
...
def get_dag_by_file_location(dag_id: str):
    ...
    if _should_read_dag_from_db(dag_model.fileloc):
...
def get_dag(subdir: str | None, dag_id: str, from_db: bool = False) -> DAG:
    ...
    if from_db or _should_read_dag_from_db(subdir):
        dagbag = DagBag(read_dags_from_db=True)
