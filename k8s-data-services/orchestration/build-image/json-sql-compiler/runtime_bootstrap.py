...
def build_spark_conf(task_var: dict[str, Any]) -> dict[str, Any]:
    conf = dict(task_var.get("conf") or {})

    default_container_image = os.getenv(
        "JSON_SQL_SPARK_CONTAINER_IMAGE",
        "ghostwood/spark-engine:1.0.1",
    )
    default_service_account = os.getenv("JSON_SQL_SPARK_SERVICE_ACCOUNT", "spark")

    if default_container_image and "spark.kubernetes.container.image" not in conf:
        conf["spark.kubernetes.container.image"] = default_container_image

    service_account_key = "spark.kubernetes.authenticate.driver.serviceAccountName"
    if default_service_account and service_account_key not in conf:
        conf[service_account_key] = default_service_account
...
def build_operator_from_task_var(task_var: dict[str, Any]) -> SparkSubmitOperator:
    ...
    kwargs = {
        ...
        "conf": build_spark_conf(task_var),
    }
...
def set_task_instance_state(
    ...
    if not run_id:
        print("task_instance_state_update: skipped because run_id is missing")
        return
    ...
    try:
        ...
        if task_instance is None:
            print(
                "task_instance_state_update: task instance not found:",
                dag_id,
                task_id,
                run_id,
            )
        ...
        old_state = task_instance.state
        task_instance.state = state

        if state == TaskInstanceState.RUNNING and task_instance.start_date is None:
        ...
        session.rollback()
        print("task_instance_state_update: ERROR:", repr(exc))
        raise
