...
These files must be available inside Airflow pods under:
...
The `.airflowignore` file should exclude runtime helper code from normal DAG
parsing:
...
```text
python /opt/airflow/dags/json-sql-compiler/runtime_bootstrap.py --dag-id ... --task-id ... --run-id ... --execute
```
